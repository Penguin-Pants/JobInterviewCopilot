...
export const OVERLAY_SIZE = { width: 420, height: 260 } as const;
...
export function resolveOverlayPosition(
  ...
  return {
    x: Math.round(primary.bounds.x + primary.bounds.width - OVERLAY_SIZE.width - 48),
...
/** Persist the overlay's current position and display (FR-082, CH-119). */
export function saveOverlayPosition(win: BrowserWindow, config: ConfigStore): void {
  if (win.isDestroyed()) return;
  const [x, y] = win.getPosition();
  if (x === undefined || y === undefined) return;
  const display = screen.getDisplayNearestPoint({ x, y });
  config.set({ overlayWindow: { x, y, displayId: String(display.id) } });
}
/* v8 ignore stop */
