...
| TC-184 | U | classify() health-check and timeout policy | With the LLM primary in `CONFIG_REQUIRED`, or in `DEGRADED` with backoff not yet due, `classify()` makes zero network calls and resolves `'actionable'` immediately. With the primary healthy, exactly one attempt is made under an 800 ms client-side timeout (`ASM-020`); a scripted delay past it resolves `'actionable'` and reports no probe outcome to the fake `CMP-12` health tracker either way |
