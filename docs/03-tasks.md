...
| Criterion | Proved by |
|---|---|
| Readiness after the reminder renders, nothing dropped | Two halves, and neither claims the other. `OverlayGate` is driven directly in `tests/unit/overlay-surface.test.ts`: buffer rather than drop, a second generation discards the first, a rebuilt renderer is replayed in full. `TC-138` asserts only what needs a window: a whole generation replayed to a renderer that never saw it live is reconstructed into a card, a second generation is its own card, and a line whose begin never arrived is dropped. `TC-138` sends through `webContents.send`, so it does not exercise the gate itself, and the reason it cannot is carried below |
...
**Defects found by the Codex review on the pull request, all five verified and
fixed** across two rounds. Three P1 and two P2. The first is the one this task's
own acceptance criteria are built around, and neither of the two reviews before
it caught it.

| Defect | Consequence | Fix |
|---|---|---|
| **P1. The readiness gate was armed once per renderer, not once per session.** `reported.current` latched after the first `overlay:ready` and `OverlayGate.reset()` cleared the card while leaving `ready` true | `FR-006` asks for the reminder before the first suggestion of **every** live session, and `FR-008` gates delivery on it having rendered. Answered once at the first load, that question stood for the life of the window: the reminder is dismissible, so by the second interview it was off screen, and the renderer re-shows it on a `state:session` push React processes asynchronously. The gate then delivered the second interview's first suggestion on whatever had been painted, which is exactly the race `ADR-016` exists so that nothing has to win. Not practically reachable, because a suggestion needs a turn plus two round trips while the re-render is two commits, but a guarantee that rests on winning a race is not the guarantee `FR-008` states | `reset()` clears readiness as well as the card, and the renderer re-reports once the renewed reminder has painted, keyed on a session epoch rather than a latching ref. Until it does, the next interview's suggestions buffer rather than arriving early, which is what the gate does at every other closed moment. Pinned by a gate test that drives a whole second interview through the boundary |
| **The renderer could miss a session boundary entirely**, found by the second Windows CI run. It detected a new interview by watching `active` go false and back to true on `CH-201` | Two pushes delivered close enough together land in **one** React batch: `active` is then true before and after, React bails out of the render, and the effect never runs. Measured on the built renderer, that is exactly what happened, and the second interview began with the reminder still dismissed from the first, which is the `FR-006` requirement this task exists to satisfy. A window rebuilt across the boundary loses the gap the same way | The boundary is keyed on the **session id**, which is what a session is and therefore cannot be missed: it differs whether or not a moment with no session running was ever observed. A re-push carrying the same id is still not a boundary, which is what keeps crash recovery from wiping a card mid-interview. `setSession` in the E2E suite now names its sessions apart, because reusing one id was one session pushed twice rather than the two `TC-006` claims to drive |
| **P1. `overlay:setFontSize` was range-checked but not rate-limited.** The no-op guard stops a renderer looping one value; alternating two defeats it | Every alternating call reached `config.set`, a **synchronous** settings write on the same event loop as the live audio and STT loop. The one narrow write the overlay can reach became a way for a compromised renderer to stall an interview, which is worse than anything the allowlist was narrowed to prevent (`FR-086`) | A leading-edge throttle with a trailing commit, in `src/main/write-throttle.ts` with injected timers so it is driven with fake timers rather than waited for: the first press writes at once, any number of calls inside the window cost one write, and the last value asked for is the one stored. The renderer gained a draft at the same time, because coalescing means the stored size no longer moves on every press and each press would otherwise step from a stale number: three quick presses from 26 all asked for 28. Measured on the built renderer with the throttle in place: five rapid presses move 22 to 32, not 24 |
| **P1. Only the window closing re-armed the gate, not a document reload.** `closed` fires when the window goes away; nothing fired when the document was replaced under a window that stays | The gate's invariant is about the **document**: `ready` means this renderer has painted the consent reminder. A new document reports ready two animation frames after it mounts, and a gate still holding the old document's answer delivers into that interval: the messages reach a page with no subscription, `delivered` advances past them, and the begin is then never replayed, so the lines that follow arrive with no card to render them on. Nothing in this app reloads the overlay, so this is an invariant repaired rather than a bug reproduced, but a crashed renderer and `TC-115`, which reloads to pick up an emulated `prefers-reduced-motion`, both take that path | `did-start-loading` on the overlay's `webContents` calls `noteClosed()`, which is exactly the right call: it keeps the card and resets only how much of it this renderer has been sent, so a generation streaming across the reload is replayed in full (ADR-015). Pinned by a guardrail, as every `index.ts` behaviour is |
| **P2. A queued font write could reverse a newer Dashboard change.** The throttle's commit re-reads the theme when it fires, so it picked up the Dashboard's newer values for every field **except** the one it was holding | A press on the overlay control followed within 200 ms by a Dashboard commit stored the older overlay size, undoing the user's last action. Last-write-wins, inverted | `config:set` cancels a pending overlay write when the patch carries a theme. The queued value is older by construction, so it is dropped rather than delayed: only the newest writer commits |
| **P2. `TC-115` observed only the settled opacity** | A bullet that was never animated also ends at 1, so the case passed against a build that had dropped the fade `NFR-010` keeps. It asserted its own name rather than its requirement | A frame sampler installed **before** the bullet exists records the lowest opacity seen and whether any transform was ever applied. `TC-115` now asserts the fade happened and the slide did not; `TC-112` asserts both happened, so the positive case is no longer left to inference either. Measured: normal motion 0 and true, reduced motion 0 and false |
...
| **The reveal evidence was sampled with `requestAnimationFrame`**, which the runner does not deliver dependably. The overlay window is never shown during the E2E suite, because nothing there starts a real session and `showInactive` is what reveals it | `TC-112` reported "the bullet never faded in" on the runner while recording the reveal on every local run, and `TC-115`, which uses the same sampler, passed. An assertion that depends on frames arriving is not an assertion about the code | A `MutationObserver` instead: records are delivered on the microtask checkpoint, so they arrive whether or not the compositor is running, and watching the `style` attribute makes every write framer-motion performs one of them. It also reads the **declared** animation's keyframes, which exist from the moment it starts and need no frame to have elapsed, and it fails honestly when it observed nothing at all rather than passing vacuously |
| **Three assertions read a frame rather than the state under test**, found by the first Windows CI run: `TC-006` measured the newest card's bottom against the viewport while the card was still animating into place | Measured on the built renderer: the card's bottom is 331 while it travels and 252 once it arrives, and CI caught it at 332. The geometry was right and the assertion was not | The geometry claims are polled, so they are about the settled layout. The reduced-motion transform read is polled for the same reason |
| **`TC-113` read the in-overlay size readout before the control existed.** The control is rendered in interactive mode only, and the reorder that fixed the stale-draft problem moved the read ahead of the mode push | Reported as "element not found" on CI, which is a test defect wearing the costume of a product one | The readout is asserted after the mode push |
| **`TC-116` matched one serialisation of a colour.** `color-mix(in srgb, …)` computes to `color(srgb r g b / a)` in current Chromium, not to `rgba(…)` | The assertion failed against a perfectly correct card. Which form a browser emits is its business, not the card's | Both forms are accepted, and the card is separately asserted not to be fully transparent, which is the claim that has content |
| **`TC-113` assumed the Dashboard's slider tracked a size written from the overlay.** Nothing pushes settings to the Dashboard, and `locator.focus()` focuses an element rather than firing the window `focus` event the re-read is bound to | The Dashboard half ran against a draft still holding 22, so four arrow presses produced 26 where the case asserted 20: it would have failed on CI, or been flaky if a window focus happened to fire and race the keypresses | The Dashboard control is driven **first**, while its draft is in step with the stored value, and the overlay control takes over from there. The case is about `FR-093`'s two controls, so it does not walk into a gap that is carried as follow-up work |
| **`TC-116` rebound to the window that was about to be destroyed.** A translucency change destroys the overlay on the far side of a `config:set` round trip, so the doomed page was still open and still matched when `selectOption` returned | The assertion would have polled a dead renderer until the timeout, or thrown "target page has been closed" mid-assertion. A test that cannot pass is worse than no test | `overlayPage` takes the page it is replacing and waits for one that is not it. The case also asserts the old page really closed, rather than asserting a window count that cannot fail |
| **Two assertions in `TC-116` could not fail.** A stylesheet element exists in any Vite build, and `app.windows().length > 0` is true for as long as the test is running | They read as proof of `FR-094` and of "no restart" and were neither | The card's resolved background colour is asserted instead, which is the custom property `theme.ts` computed arriving through the Tailwind stylesheet, and the restart claim is the old page being closed while the Dashboard that drove the change is still up |
...
| **The first card of every active period, and all of its bullets, skipped the `FR-092` reveal.** `AnimatePresence` carried `initial={false}`, and it mounts with its first child because the stack is only rendered once a card exists. On its first render that prop reaches the child as `initial: false`, memoised on the presence context without `initial` among its dependencies, so it stuck for that card's life and reached every `BulletReveal` inside it | Measured on the built renderer: card one appeared at full opacity with no fade and no slide while cards two and three animated correctly, and it recurred after every pause and every session boundary, because those unmount the stack. `FR-092` is precisely that reveal, and no assertion would have caught it: the E2E case polls the bullet's **final** opacity, which a suppressed animation satisfies trivially | `initial={false}` is gone. Re-measured: all four cases reveal, and under `prefers-reduced-motion` all four fade without sliding |
| **The Dashboard's `did-finish-load` replay had never fired.** `wireDashboardWindow()` ran after `await createDashboardWindow(...)`, and `loadFile` resolves from **inside** `did-finish-load`, so the listener was attached to an event that had already been emitted | Pre-existing, from `TASK-042`, and this task made it user-visible: with `CH-216` delivered per window, a Dashboard that never hears its own load never learns `acrylicSupported`, so the acrylic option would have been permanently disabled on Windows 11 with a note saying the machine cannot render it. The same dead listener also drops `model:download`, `state:providers` and `state:session` | `createDashboardWindow` takes the `onCreated` callback `createOverlayWindow` already had, for the reason `createOverlayWindow`'s own comment gives, and all three call sites wire inside it. `TC-142` asserts the acrylic option is offered with nothing pushed by the test, which fails under the old ordering |
| **The alpha-floor search assumed worst-case contrast rises with alpha.** It does not: the minimum over the two backdrops rises while the card covers the hostile one, then falls back toward the card's own contrast at alpha 1, so the passing set is an interval rather than a suffix of [0, 1] | Latent rather than live, and verified so: both shipped palettes hold across the whole band above their floors, at 4.579 and 4.557. But a floor found by stopping at the first pass can have a failing band above it, and `cardSurfaceAlpha` hands back any user opacity at or above the floor, so a palette tweak could have landed in that band with `TC-114` still green | The scan runs **downward from 1** and stops at the first failure, which makes the band above the answer safe by construction. Two tests pin it: every alpha from the floor to 1 clears the target, and a deliberately non-monotonic palette does not get a floor with a failing band above it |
| **`textContrast` reported the colour that is not the binding constraint.** It measured `text`, while `muted` is what the floor is pinned to and what renders a card's question line and the idle message | The field's own name says "the worst contrast", and it overstated it by about half again, so a palette change that pushed `muted` to 3 to 1 would have left the assertion on it comfortably passing | It is the minimum over the held set, and a test asserts it equals the `muted` figure rather than the `text` one |
...
| An overlay rebuilt mid-session brings the consent card back after the user dismissed it | `dismissed` is renderer state and nothing carries it across a rebuild. Re-showing is the conservative direction for `FR-006`, which is why it is not a defect, but it is an annoyance on a translucency change. Carrying the dismissal would mean the main process holding it for the session, which is where `CH-120`'s new handler could keep it | TASK-050 |
...
**Blocker, confirmed 2026-09-23 after Milestone 6 was completed.** This is now
the only open item in the whole build plan. It cannot be done from a Linux
