...
| `overlay:setFontSize` has no rate limit | A no-op write is dropped, which is the cheap half and removes the same-value loop, but a renderer alternating two sizes still drives one synchronous settings write per call on the process that runs the live audio and STT loop. The Dashboard's control debounces to one write per adjustment; matching that here means a main-process debounce, which is a change to how every setting is written | TASK-050 |
